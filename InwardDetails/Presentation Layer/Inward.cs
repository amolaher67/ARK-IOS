    ...
    public partial class PaymentVoucher : Form
    {
        ...
        private void Save()
        {
            try
            {
                ...
                var customrGrn = txt_GrnNo.Text;
                if (!string.IsNullOrEmpty(customrGrn))
                {
                    //Check wheather this Grn No is correct ,it should not be duplicate
                    CSInward objCsInward = new CSInward();
                    objCsInward.InwardNumber = txt_GrnNo.Text;
                    if (objCsInward.CheckGrnisDuplicateinThisFinicialYear() == true)
                    {
                        MessageBox.Show("This GRNO is allready used Select different Grn no in same finicial year", "Inward", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        txt_GrnNo.Focus();
                        return;
                    }
                }
                ...
                if (MessageBox.Show("Do you want to save Inward ?", "IOS", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    obj.InwardNumber = txt_GrnNo.Text.Trim();
                    ...
                    obj.CurrentFinicialYearID = GlobalData.CurrentFinicialYearID;

                    if (txt_PONO.Text.Trim() != String.Empty)
                        obj.PODate = dtp_PO.Value.Date;
                    else
                        obj.PODate = null;
                    if (IsNew == true)
                    {
        ...
        public void newUpdate()
        {
            ...
            txt_GrnNo.Text = Convert.ToString(obj.getMaxNumber(GlobalData.CurrentFinicialYearID));
        }
        ...
        /// <summary>
        /// On leave GRN textBox will check does that GRN is valid or not,if it is entered by user
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void txt_GrnNo_Leave(object sender, EventArgs e)
        {
